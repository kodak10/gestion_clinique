...
class TracabiliteController extends Controller
{
    ...
    public function getTracabiliteData(Request $request)
    {
        ...
        return DataTables::eloquent($activities)
            ...
            // Filtre personnalisé pour la recherche FR
            ->filterColumn('action', function($query, $keyword) {
                $keyword = strtolower($keyword);
                if (str_contains($keyword, 'supprimer') || str_contains($keyword, 'suppression')) {
                    $query->where('event', 'deleted');
                } elseif (str_contains($keyword, 'modifier') || str_contains($keyword, 'modification')) {
                    $query->where('event', 'updated');
                } elseif (str_contains($keyword, 'créer') || str_contains($keyword, 'creation') || str_contains($keyword, 'creer')) {
                    $query->where('event', 'created');
                } elseif (str_contains($keyword, 'restaurer') || str_contains($keyword, 'restauration')) {
                    $query->where('event', 'restored');
                }
            })
            // Colonne Modèle
            ->addColumn('model', function($activity) {
